...
async function main() {
  ...
  console.log(`${COLORS.blue}Building all projects...${COLORS.reset}\n`);
  ...
  for (const dir of topDirs) {
    ...
    try {
      // Auto-install dependencies before building if needed
      const hasNodeModules = await (async () => {
        try {
          await fsp.access(path.join(dirPath, "node_modules"));
          return true;
        } catch {
          return false;
        }
      ...
      if (!hasNodeModules) {
        ...
        execSync(`cd "${dirPath}" && npm install`, { stdio: "pipe" });
      }

      execSync(`cd "${dirPath}" && npm run build`, { stdio: "inherit" });
    } catch (err) {
      console.error(`  ${COLORS.yellow}Build failed in ${dir}${COLORS.reset}`);
  ...
  if (skipped.length > 0) console.log(`  Skipped (no build script): ${skipped.length}`);
  if (failed.length > 0) {
    console.log(`  ${COLORS.yellow}Failed: ${failed.length} (${failed.join(", ")})${COLORS.reset}`);
    process.exit(1);
  } else {
    console.log(`\n${COLORS.green}All builds complete!${COLORS.reset}`);
  }
}
